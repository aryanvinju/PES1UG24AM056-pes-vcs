...
static int write_tree_level(const Index *index, const char *prefix, ObjectID *id_out) {
    ...
    for (int i = 0; i < index->count; i++) {
        ...
        if (prefix_len > 0) {
            if (strncmp(rel, prefix, prefix_len) != 0) continue;
            rel += prefix_len;
        }
        ...
        if (!slash) {
            ...
            out->mode = entry->mode;
            out->hash = entry->hash;
            snprintf(out->name, sizeof(out->name), "%s", rel);
            continue;
        }
        ...
        for (int j = 0; j < seen_count; j++) {
            if (strncmp(seen_dirs[j], rel, dir_len) == 0 && seen_dirs[j][dir_len] == '\0') {
                already_seen = 1;
                break;
            }
        }
        ...
        memcpy(seen_dirs[seen_count], rel, dir_len);
        seen_dirs[seen_count][dir_len] = '\0';
        seen_count++;

        char child_prefix[1024];
        snprintf(child_prefix, sizeof(child_prefix), "%s%.*s/", prefix, (int)dir_len, rel);
        ...
        out->mode = 0040000;
        out->hash = child_id;
        snprintf(out->name, sizeof(out->name), "%.*s", (int)dir_len, rel);
    }
